...
__kernel void TetrahedraScatter( __global int4 *TetVer, __global float4 (*TetPos)[4], \
...
__kernel void TetrahedraGather1( __global char *VerDeg, __global int16 (*VerBal)[2], \
                                 __global float4 (*TetPos)[4], __global float4 *VerCrd, \
                                 __global GmlParSct *par, const int count )
{
   ...
   VerIdx = BalCod & (int16){7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7};
...
__kernel void TetrahedraGather2( __global int (*ExtDeg)[3], __global int *ExtBal, \
                                 __global float4 (*TetPos)[4], __global float4 *VerCrd, \
                                 __global GmlParSct *par, const int count )
{
   ...
   NewCrd = VerCrd[ VerIdx ] * (float4){32,32,32,0}; 
