...
__kernel void TrianglesBasic( __global int4 *TriVer, __global float4 *MidTri, \
                              ...
                              const int count )
{
   ...
   MidTri[i] = (VerCrd[ idx.s0 ] + VerCrd[ idx.s1 ] + VerCrd[ idx.s2 ]) \
               * (float4){.333333,.333333,.333333,0};
}
