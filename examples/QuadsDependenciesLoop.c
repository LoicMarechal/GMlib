...
{
	int iter, i, j, NmbVer, NmbQad, InpMsh, ver=0, dim=0, ref, (*QadTab)[4]=NULL, GpuIdx=0;
	int VerIdx, QadIdx, CalPos, CalCrd1, CalCrd2, BalIdx, PosIdx, NmbItr=100;
	...
	/* Open the mesh */
	if( !(InpMsh = GmfOpenMesh("quads.meshb", GmfRead, &ver, &dim)) || (ver != GmfFloat) || (dim != 3) )
		return(1);
	...
	if( !(NmbQad = GmfStatKwd(InpMsh, GmfQuadrilaterals)) || !(QadTab = malloc((NmbQad+1) * 4 * sizeof(int))) )
		return(1);
	...
	for(i=1;i<=NmbQad;i++)
		GmfGetLin(InpMsh, GmfQuadrilaterals, &QadTab[i][0], &QadTab[i][1], &QadTab[i][2], &QadTab[i][3], &ref);
	...
	for(i=1;i<=NmbQad;i++)
		GmlSetQuadrilateral(QadIdx, i-1, QadTab[i][0]-1, QadTab[i][1]-1, QadTab[i][2]-1, QadTab[i][3]-1);
