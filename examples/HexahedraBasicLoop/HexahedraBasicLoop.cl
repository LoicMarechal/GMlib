...
__kernel void HexahedraBasic(	__global int8 *HexVer, __global float4 *MidHex, __global float4 *VerCrd, \
								__global GmlParSct *par, const int count )
{
	...
	MidHex[i] = (VerCrd[ idx.s0 ] + VerCrd[ idx.s1 ] + VerCrd[ idx.s2 ] + VerCrd[ idx.s3 ] + VerCrd[ idx.s4 ] + \
				VerCrd[ idx.s5 ] + VerCrd[ idx.s6 ] + VerCrd[ idx.s7 ]) * (float4){.125,.125,.125,0};
}
