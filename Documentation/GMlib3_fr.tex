...
\normalsize Juillet 2020, document v2.10, librairie v3.30
\end{flushright}
...
Les trois principaux constructeurs, \emph{AMD}, \emph{Intel} et \emph{Nvidia}, proposent tous gratuitement des kits de développement GPU pour Linux et Windows10 dont les références se trouvent ici : \cite{driver_amd}, \cite{driver_intel}, \cite{driver_nvidia}.
...
Pour contourner le problème, il existe de nombreuses techniques, comme la découpe en sous-blocs de triangles n'ayant aucun sommet en commun (cf. \cite{lplib}), les micro synchronisations afin d'éviter les conflits d'écritures simultanées (cf. \cite{atomic}) et enfin la duplication des données : c'est la méthode scatter-gather, idéale pour les GPU.
...
Il est aussi intéressant de lire quelques documents parlant d'optimisation des codes en OpenCL (cf. \cite{nvidia} et \cite{apple}) ou d'expérience de portage de codes préexistants sur GPU (cf. \cite{lohner}).
