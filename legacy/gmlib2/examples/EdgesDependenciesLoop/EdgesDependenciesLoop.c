...
{
	int iter, i, j, NmbVer, NmbEdg, InpMsh, ver=0, dim=0, ref, (*EdgTab)[2]=NULL, GpuIdx=0;
	int VerIdx, EdgIdx, CalPos, CalCrd1, CalCrd2, BalIdx, PosIdx, NmbItr=100;
	...
	/* Open the mesh */
	if( !(InpMsh = GmfOpenMesh("edges.meshb", GmfRead, &ver, &dim)) || (ver != GmfFloat) || (dim != 3) )
		return(1);
	...
	if( !(NmbEdg = GmfStatKwd(InpMsh, GmfEdges)) || !(EdgTab = malloc((NmbEdg+1) * 2 * sizeof(int))) )
		return(1);
